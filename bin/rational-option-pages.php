 ...
 *
 * Note this has been significantly modified because it was very buggy...
 *
...
namespace StaffingEngine\Chatbot\Bin;
...
class RationalOptionPages {
					...
					if ( ! empty( $section_params['fields'] ) ) {
						foreach ( $section_params['fields'] as $field_key => $field_params ) {
							...
							if (
								...
								$params['title'] = "<label for='{$params["id"]}'>" . esc_html( $params['title'] ) . "</label>";
							}
	...
	public function admin_notices() {
		...
		if ( ! empty( $this->escaped_errors ) ) {
			foreach ( $this->escaped_errors as $escaped_error ) {
				echo wp_kses( $escaped_error, array() );
			}
		}
		if ( ! empty( $this->escaped_notices ) ) {
			foreach ( $this->escaped_notices as $escaped_notice ) {
				echo wp_kses( $escaped_notice, array() );
			}
		}
	...
	protected function build_menu_page( $page_key ) {
		...
		settings_errors();
		?>
		<div class="wrap" id="<?php echo esc_html( $page['menu_slug']  . '-page'); ?>">
	...
	protected function build_settings_field( $page_key, $section_key, $field_key ) {
		...
		if ( ( ! isset( $field['sanitize'] ) || $field['sanitize'] ) && $field['type'] !== 'wp_editor' ) {
			if ( ! empty( $field['attributes'] ) && isset( $field['attributes']['multiple'] ) && $field['attributes']['multiple'] ) {
				foreach ( $field['value'] as $i => $iValue ) {
					$field['value'][ $i ] = strip_tags( $iValue );                 // Removes HTML tags
		...
		switch ( $field['type'] ) {
			case 'checkbox':
				...
				printf(
					...
					! empty( $field['class'] ) ? esc_attr( "class={$field['class']}" ) : '',          // class
					esc_attr( $field['id'] ),                                                           // id
					esc_attr( "{$page_key}[{$field['id']}]" ),                                          // name
					esc_attr( $field['title_attr'] ),                                                   // title
					esc_attr( $field['value'] ),                                                        // value
					! empty( $field['text'] ) ? wp_kses_post( $field['text'] ) : ''                         // text
				);
        if ( ! empty( $field['description'] ) ) {
          printf(
            '<p class="se-checkbox-description">%s</p>',
			...
			case 'media':
				...
				printf(
					...
					esc_attr( $field['id'] ),                                                                           // id
					esc_attr( "{$page_key}[{$field['id']}] " ),                                                          // name
					! empty( $field['placeholder'] ) ? 'placeholder="' . esc_html( $field['placeholder'] ) . '"' : '',  // placeholder
					esc_attr( $field['title_attr'] ),                                                                   // title
					esc_attr( $field['value'] ),                                                                        // value
					! empty( $attributes ) ? esc_attr( implode( ' ', $attributes ) ) : '',                              // additional attributes
					$upload_button,                                                                                     // upload button
					! empty( $field['text'] ) ? '<p class="help">' . wp_kses_post( $field['text'] ) . '</p>' : ''           // text
				);
				break;
			case 'radio':
				...
				foreach ( $field['choices'] as $value => $label ) {
					...
					printf(
						...
						esc_attr( $field['id'] ),                                                   // id
						esc_attr( "{$page_key}[{$field['id']}]" ),                                  // name
						esc_html( $label, 'staffing-engine-chatbot' ),                              // title
						esc_attr( $value ),                                                         // value
						esc_html( $label, 'staffing-engine-chatbot' ),                              // label
						$c < count( $field['choices'] ) - 1 ? '<br>' : ''                           // line-break
					);
					$c ++;
				}
				echo '</fieldset>';
        echo ! empty( $field['text'] ) ? '<p class="help">' . wp_kses_post( $field['text'] ) . '</p>' : ''; // title
				break;
			case 'select':
				if ( ! empty( $field['attributes'] ) && isset( $field['attributes']['multiple'] ) && $field['attributes']['multiple'] ) {
				...
				printf(
					...
					! empty( $field['class'] ) ? esc_attr( "class={$field['class']}" ) : '',  // class
					! empty( $attributes ) ? $this->print_attributes($attributes) : '',
					esc_attr( $field['id'] ),                                                   // id
					esc_attr( $field_tag_name ),                                                // name
					esc_html( $field['title_attr'] ),
				);
				foreach ( $field['choices'] as $value => $text ) {
					...
					printf(
						...
						esc_attr( $value ),                              // value
						esc_html( $text, 'staffing-engine-chatbot' )     // text
					);
				}
				echo '</select>';
        echo ! empty( $field['text'] ) ? '<p class="help">' . wp_kses_post( $field['text'] ) . '</p>' : ''; // title
			...
			case 'textarea':
				printf(
					...
					! empty( $field['placeholder'] ) ? 'placeholder="' . esc_html( $field['placeholder'] ) . '"' : '',  // placeholder
					! empty( $field['rows'] ) ? esc_attr( "rows={$field['rows']}" ) : '',                             // rows
					! empty( $field['cols'] ) ? esc_attr( "cols={$field['cols']}" ) : '',                             // cols
					! empty( $field['wrap'] ) ? esc_attr( "wrap={$field['wrap']}" ) : '',                             // wrap
					esc_attr( $field['title_attr'] ),                                                                   // title
					esc_attr( $field['value'] ),                                                                        // value
					! empty( $field['text'] ) ? '<p class="help">' . wp_kses_post( $field['text'] ) . '</p>' : ''           // text
				);
			...
			case 'wp_editor':
				...
				wp_editor( $field['value'] ?? '', $field['id'], array(
					'textarea_name' => esc_attr( $field['textarea_name'] ),
				) );
				echo ! empty( $field['text'] ) ? '<p class="help">' . wp_kses_post( $field['text'] ) . '</p>' : '';
				break;
			default:
				printf(
  ...
  private function print_attributes( $attributes ) {
    $result = '';
	...
	protected function build_settings_section( $page_key, $section_key ) {
		...
		echo ! empty( $section['text'] ) ? wp_kses( '<p id="section-description-' . $section['id'] . '">' . $section['text'] . '</p>', 'post' ) : '';
	 ...
	 *
	 * @param string $escaped_error The error to be output
	 */
	public function submit_error( $escaped_error ) {
		$escaped_error = sprintf(
			'<div class="notice notice-error"><p>%s</p></div>',
			is_array( $escaped_error ) || is_object( $escaped_error ) ? '<pre>' . htmlspecialchars( print_r( $escaped_error, true ) ) . '</pre>' : $escaped_error
		);
		if ( empty( $this->points['admin_notices'] ) ) {
			$this->escaped_errors[] = $escaped_error;
		} else {
			echo wp_kses( $escaped_error, array() );
		}
	}
	 ...
	 *
	 * @param string $escaped_notice The text to be output
	 */
	public function submit_notice( $escaped_notice ) {
		$escaped_notice = sprintf(
			'<div class="notice notice-info"><p>%s</p></div>',
			is_array( $escaped_notice ) || is_object( $escaped_notice ) ? '<pre>' . htmlspecialchars( print_r( $escaped_notice, true ) ) . '</pre>' : $escaped_notice
		);
		if ( empty( $this->points['admin_notices'] ) ) {
			$this->escaped_notices[] = $escaped_notice;
		} else {
			echo wp_kses( $escaped_notice, array() );
		}
	}
	...
	protected function validate_field( $field, $page_key, $section_key, $field_key, $page, $section ) {
		...
		if ( empty( $field['title'] ) ) {
			$this->submit_error( esc_html__( 'Field parameter "title" is required', 'staffing-engine-chatbot' ) );
		}
		...
		// Title attribute
		$field['title_attr'] = empty( $field['title_attr'] ) ? esc_html( $field['title'] ) : $field['title_attr'];
	...
	protected function validate_page( $page_key, $page_params, $parent_slug = false ) {
		...
		if ( empty( $page_params['page_title'] ) ) {
			$this->submit_error( esc_html__( 'Page parameter "page_title" is required', 'staffing-engine-chatbot' ) );
		}

		// Menu title
		if ( empty( $page_params['menu_title'] ) ) {
			$page_params['menu_title'] = esc_html( $page_params['page_title'] );
		}
		...
		if ( empty( $page_params['parent_slug'] ) && ! $parent_slug ) {
			...
			$page_params['parent_slug'] = $parent_slug ?: $page_params['parent_slug'];
		}
	...
	protected function validate_section( $section, $page_key, $section_key, $page ) {
		...
		if ( empty( $section['title'] ) ) {
			$this->submit_error( esc_html__( 'Section parameter "title" is required', 'staffing-engine-chatbot' ) );
		}
